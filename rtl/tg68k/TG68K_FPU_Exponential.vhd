...
architecture rtl of TG68K_FPU_Exponential is
	...
	constant DIRECT_ZERO : std_logic_vector(2 downto 0) := "000";
	constant DIRECT_ONE : std_logic_vector(2 downto 0) := "001";
	constant DIRECT_ONES : std_logic_vector(2 downto 0) := "010";
	constant DIRECT_ONE_STICKY : std_logic_vector(2 downto 0) := "011";
	constant DIRECT_SOURCE_PLUS_ONE : std_logic_vector(2 downto 0) := "100";
	constant DIRECT_SOURCE_MINUS_ONE : std_logic_vector(2 downto 0) := "101";
	constant DIRECT_SOURCE_MINUS_ONE_SHIFT : std_logic_vector(2 downto 0) :=
		"110";
	...
	-- Reuse the CORDIC result pair for subtraction and tangent division.
	signal value_register : unsigned(CORDIC_WIDTH downto 0) :=
		(others => '0');
	signal secondary_value_register : unsigned(CORDIC_WIDTH downto 0) :=
		(others => '0');
	...
	signal direct_significand_kind : std_logic_vector(2 downto 0) := DIRECT_ZERO;
	signal direct_significand_value : fpu_significand_grs_t;
...
begin
	...
	selected_significand <= intermediate_significand when
		use_iterative_significand = '1' else direct_significand_value;
	round_input.significand <= selected_significand;
	round_input.special <= intermediate_special;
	base_exception_status <= base_status;

	direct_result : process(direct_significand_kind,
			series_source_significand)
		variable source_value : fpu_significand_grs_t;
	begin
		source_value := shift_left(resize(series_source_significand,
			source_value'length), 3);
		case direct_significand_kind is
			when DIRECT_ONE =>
				direct_significand_value <= (66 => '1', others => '0');
			when DIRECT_ONES =>
				direct_significand_value <= (others => '1');
			when DIRECT_ONE_STICKY =>
				direct_significand_value <=
					(66 => '1', 0 => '1', others => '0');
			when DIRECT_SOURCE_PLUS_ONE =>
				direct_significand_value <= source_value + 1;
			when DIRECT_SOURCE_MINUS_ONE =>
				direct_significand_value <= source_value - 1;
			when DIRECT_SOURCE_MINUS_ONE_SHIFT =>
				direct_significand_value <= shift_left(source_value - 1, 1);
			when others =>
				direct_significand_value <= (others => '0');
		end case;
	end process;
	...
	begin
		...
		case state is
			...
			when NORMALIZE_HYPERBOLIC_TANGENT =>
				left_value := value_register;
				if value_register >= secondary_value_register then
					right_value := secondary_value_register;
					subtract_value := '1';
				end if;
			when DIVIDE_HYPERBOLIC_TANGENT =>
				shifted_remainder := shift_left(value_register, 1);
				left_value := shifted_remainder;
				if shifted_remainder >= secondary_value_register then
					right_value := secondary_value_register;
					subtract_value := '1';
				end if;
			when others => null;
	...
	exponential_sequence : process(clk)
		...
		begin
			value_register <= normalized_value;
		...
		procedure begin_hyperbolic_tangent_division(
				...
				constant prior_sticky : in std_logic) is
		begin
			value_register <= numerator;
			secondary_value_register <= denominator;
		...
		begin
			...
			if exponent_integer < 0 then
				...
				if -exponent_integer > FRACTION_BITS then
					...
					value_register <= normalized_value;
					secondary_value_register <= one_value;
					subtraction_exponent <= (others => '0');
				...
				if exponent_integer > FRACTION_BITS then
					...
					if difference_value(FRACTION_BITS) = '1' then
						...
						value_register <= difference_value;
						subtraction_exponent <= normalized_exponent;
						subtraction_sticky <= '0';
	...
	begin
		if rising_edge(clk) then
			if nReset = '0' then
				...
				series_carry_borrow <= '0';
				series_term_subtract <= '0';
				value_register <= (others => '0');
				secondary_value_register <= (others => '0');
				subtraction_exponent <= (others => '0');
				...
				direct_significand_kind <= DIRECT_ZERO;
				...
				case state is
						...
						if start = '1' then
							...
							if source_class = FPU_CLASS_QUIET_NAN or
								...
								if subtract_one = '1' or hyperbolic_sine = '1' or
									...
									intermediate_class <= FPU_CLASS_NORMAL;
									direct_significand_kind <= DIRECT_ONE;
								end if;
								state <= COMPLETE;
								...
								if hyperbolic_tangent = '1' and
									...
									if source_exponent <= TANH_TINY_MAX_EXPONENT then
										intermediate_class <= FPU_CLASS_NORMAL;
										intermediate_sign <= source(79);
										if source_significand =
												x"8000000000000000" then
											intermediate_exponent <= to_signed(
												source_exponent - 1, 17);
											direct_significand_kind <=
												DIRECT_SOURCE_MINUS_ONE_SHIFT;
										else
											intermediate_exponent <= to_signed(
												source_exponent, 17);
											direct_significand_kind <=
												DIRECT_SOURCE_MINUS_ONE;
										end if;
										state <= COMPLETE;
									else
									...
									if source_exponent <= COSH_TINY_MAX_EXPONENT then
										intermediate_class <= FPU_CLASS_NORMAL;
										direct_significand_kind <= DIRECT_ONE_STICKY;
										state <= COMPLETE;
									else
										series_exponent <= to_signed(source_exponent, 17);
										series_multiplier <= (others => '0');
									...
									if (exponential_base = FPU_EXP_BASE_TWO and
										...
										if hyperbolic_tangent = '1' then
											intermediate_sign <= source(79);
											intermediate_exponent <= to_signed(-1, 17);
											direct_significand_kind <= DIRECT_ONES;
										elsif hyperbolic_sine = '1' then
											intermediate_sign <= source(79);
						...
						if multiply_index = FRACTION_BITS - 1 then
							if next_accumulator = 0 then
								...
								if hyperbolic_sine_latched = '1' or
										hyperbolic_cosine_latched = '1' or
										hyperbolic_tangent_latched = '1' then
									value_register <= unit_result;
									secondary_value_register <= unit_result;
									subtraction_sticky <= '1';
									state <= COMPLETE_HYPERBOLIC;
								else
					...
					when FORM_CORDIC_SUM =>
						...
						if hyperbolic_sine_latched = '1' or
								hyperbolic_cosine_latched = '1' or
								hyperbolic_tangent_latched = '1' then
							value_register <= unsigned(cordic_sum);
					...
					when FORM_CORDIC_DIFFERENCE =>
						secondary_value_register <= value_arithmetic_result(
							CORDIC_WIDTH downto 0);
						subtraction_sticky <= '0';
						state <= COMPLETE_HYPERBOLIC;

					when COMPLETE_EXPONENTIAL =>
						complete_exponential(value_register,
							result_exponent, subtract_one_latched);

					when COMPLETE_HYPERBOLIC =>
						if hyperbolic_sine_latched = '1' then
							complete_hyperbolic_sine(value_register,
								secondary_value_register, result_exponent,
								subtraction_sticky);
						elsif hyperbolic_cosine_latched = '1' then
							complete_hyperbolic_cosine(value_register,
								secondary_value_register, result_exponent,
								subtraction_sticky);
						else
							complete_hyperbolic_tangent(value_register,
								secondary_value_register, result_exponent,
								subtraction_sticky);
						end if;
					...
					when NORMALIZE_HYPERBOLIC_TANGENT =>
						if value_register >= secondary_value_register then
							value_register <= value_arithmetic_result(
								CORDIC_WIDTH downto 0);
							...
							value_register <= shift_left(value_register, 1);
					...
					when DIVIDE_HYPERBOLIC_TANGENT =>
						...
						if tangent_shifted_remainder >=
								secondary_value_register then
							tangent_next_quotient(0) := '1';
						else
							tangent_next_quotient(0) := '0';
						end if;
						value_register <= tangent_next_remainder;
					...
					when ALIGN_SUBTRACTION =>
						...
						next_subtraction_one := secondary_value_register;
						next_subtraction_sticky := subtraction_sticky;
						if intermediate_sign = '1' then
							next_subtraction_sticky := subtraction_sticky or
								value_register(0);
							next_subtraction := shift_right(value_register, 1);
						else
							next_subtraction_one := shift_right(
								secondary_value_register, 1);
						end if;
						if subtraction_shift_count = 1 then
							if intermediate_sign = '1' then
								if next_subtraction_sticky = '1' then
									subtraction_difference :=
										secondary_value_register - next_subtraction - 1;
								else
									subtraction_difference :=
										secondary_value_register - next_subtraction;
								end if;
							else
							...
							value_register <= next_subtraction;
							secondary_value_register <= next_subtraction_one;
							subtraction_sticky <= next_subtraction_sticky;
							subtraction_shift_count <= subtraction_shift_count - 1;
						end if;
					...
					when NORMALIZE_SUBTRACTION =>
						next_subtraction := shift_left(value_register, 1);
					...
					when FORMAT_NORMALIZED_RESULT =>
						...
						intermediate_significand <= (others => '0');
						intermediate_significand(66 downto 3) <=
							value_register(FRACTION_BITS downto
								FRACTION_BITS - 63);
						intermediate_significand(2) <=
							value_register(FRACTION_BITS - 64);
						intermediate_significand(1) <=
							value_register(FRACTION_BITS - 65);
						intermediate_significand(0) <= subtraction_sticky or
							or_reduce(value_register(
								FRACTION_BITS - 66 downto 0));
						state <= COMPLETE;
