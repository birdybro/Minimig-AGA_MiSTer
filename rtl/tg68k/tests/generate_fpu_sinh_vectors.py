...
def source_values() -> list[int]:
    values = [
        encode_fraction(value) for value in (
            ...
            Fraction(-13, 4), Fraction(100), Fraction(-100),
...
def high_precision_sinh(source: int) -> Fraction:
    source_value = extended_bits_value(source)
    if abs(source_value) < Fraction(1, 1 << 200):
        return source_value + source_value * source_value * source_value / 6
...
def reference_sinh(source: int, precision_bits: int,
                   mode: int) -> tuple[int, int, int]:
    result, condition_codes, status = round_binary_precision(
        high_precision_sinh(source), precision_bits, mode)
...
def make_vectors():
    ...
    for source in source_values():
        for precision_index in range(3):
            ...
            for mode in range(4):
                result, condition_codes, status = reference_sinh(
...
entity tb_tg68k_fpu_sinh_differential is
end entity;

architecture test of tb_tg68k_fpu_sinh_differential is
    ...
    begin
        ...
        for index in vectors'range loop
            ...
            assert result = vectors(index).expected_result and
                ...
                report "differential FSINH vector " & integer'image(index) &
        ...
        report "PASS: 1152 high-precision FSINH vectors" severity note;
        stop;
    end process;
end architecture;
""")
