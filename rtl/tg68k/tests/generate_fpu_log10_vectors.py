...
def high_precision_log10(source: int) -> tuple[Fraction, bool]:
    ...
    with localcontext() as context:
        ...
        result_decimal = source_decimal.ln() / Decimal(10).ln()
    return Fraction(result_decimal), False


def reference_log10(source: int, precision_bits: int,
                    mode: int) -> tuple[int, int, int]:
    result_value, exact = high_precision_log10(source)
...
def make_vectors():
    ...
    for source in source_values():
        for precision_index in range(3):
            ...
            for mode in range(4):
                result, condition_codes, status = reference_log10(
...
entity tb_tg68k_fpu_log10_differential is
end entity;

architecture test of tb_tg68k_fpu_log10_differential is
    ...
    begin
        ...
        for index in vectors'range loop
            ...
            while done = '0' loop
                ...
                cycles := cycles + 1;
                assert cycles < 500
                    report "differential FLOG10 timeout" severity failure;
            end loop;
            assert result = vectors(index).expected_result and
                ...
                report "differential FLOG10 vector " & integer'image(index) &
        ...
        report "PASS: 1152 high-precision FLOG10 vectors" severity note;
        stop;
    end process;
end architecture;
""")
