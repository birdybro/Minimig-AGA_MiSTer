...
def high_precision_tanh(source: int) -> Fraction:
    ...
    if abs(source_value) < Fraction(1, 1 << 100):
        ...
        return source_value - source_value * square / 3 + (
            2 * source_value * square * square / 15)
    with localcontext() as context:
        ...
        result_decimal = ((positive_exponential - negative_exponential) /
                          (positive_exponential + negative_exponential))
    return Fraction(result_decimal)


def reference_tanh(source: int, precision_bits: int,
                   mode: int) -> tuple[int, int, int]:
    result, condition_codes, status = round_binary_precision(
        high_precision_tanh(source), precision_bits, mode)
...
def make_vectors():
    ...
    for source in source_values():
        for precision_index in range(3):
            ...
            for mode in range(4):
                result, condition_codes, status = reference_tanh(
...
entity tb_tg68k_fpu_tanh_differential is
end entity;

architecture test of tb_tg68k_fpu_tanh_differential is
    ...
    begin
        ...
        for index in vectors'range loop
            ...
            assert result = vectors(index).expected_result and
                ...
                report "differential FTANH vector " & integer'image(index) &
        ...
        report "PASS: 1152 high-precision FTANH vectors" severity note;
        stop;
    end process;
end architecture;
""")
