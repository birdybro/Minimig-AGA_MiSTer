...
def source_values() -> list[int]:
    values = [
        encode_fraction(value) for value in (
            ...
            Fraction(15, 16), Fraction(-15, 16),
            Fraction((1 << 32) - 1, 1 << 32),
            Fraction(-((1 << 32) - 1), 1 << 32),
            ...
            Fraction(1, 1 << 34), Fraction(-1, 1 << 34),
...
def make_vectors():
    ...
    for source in source_values():
        for precision_index in range(3):
            ...
            for mode in range(4):
                result, condition_codes, status = reference_atanh(
...
entity tb_tg68k_fpu_atanh_differential is
end entity;

architecture test of tb_tg68k_fpu_atanh_differential is
    ...
    begin
        ...
        for index in vectors'range loop
            ...
            while done = '0' loop
                ...
                cycles := cycles + 1;
                assert cycles < 520
                    report "differential FATANH timeout" severity failure;
            end loop;
            assert result = vectors(index).expected_result and
                ...
                report "differential FATANH vector " & integer'image(index) &
        ...
        report "PASS: 1152 high-precision FATANH vectors" severity note;
        stop;
    end process;
end architecture;
""")
