...
def make_vectors() -> list[tuple[int, int, str, str, int, int, int]]:
    ...
    for index in range(VECTOR_COUNT):
        ...
        source_exponent = generator.randint(-4, 5)
        ...
        if pattern == 0:
            source_sign = 0
            source_exponent = 1
            source_significand = 0xB000000000000000
        elif pattern == 1:
            source_sign = 1
            source_exponent = 0
            source_significand = 0xE000000000000000
        elif pattern == 2:
            source_exponent = -1
            source_significand = 0xFFFFFFFFFFFFFFFF
        elif pattern == 3:
            source_exponent = 5
            ...
            source_exponent = 0
            source_significand = 0x8000000000000000
            destination_significand = 0x8000008000000000
            precision_index = 1
        ...
        destination_value = extended_value(destination_sign,
                                           ...
                                           destination_significand)
        if index % 64 == 5:
            source_bits = 0x3FFF8000000000000000
        ...
        result, condition_codes, status = round_scale(
...
entity tb_tg68k_fpu_scale_differential is
end entity;

architecture test of tb_tg68k_fpu_scale_differential is
    ...
    begin
        for index in vectors'range loop
            ...
            assert result_value = vectors(index).expected_result and
                ...
                report "differential FSCALE vector " & integer'image(index) &
                    " mismatch: result=" & to_hstring(result_value) &
                    " cc=" & to_hstring(condition_codes) &
                    " status=" & to_hstring(exception_status)
                severity failure;
        end loop;
        report "PASS: 512 exact-rational FSCALE differential vectors"
            severity note;
        stop;
    end process;
end architecture;
